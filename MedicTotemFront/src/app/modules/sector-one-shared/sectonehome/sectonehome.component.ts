...
@Component({
  selector: 'app-sectonehome',
  templateUrl: './sectonehome.component.html',
  styleUrls: ['./sectonehome.component.css']
})
export class SectonehomeComponent implements OnInit {
  content?: string;
  rutCorrecto= true;
  ...
  guardarRut(): void {
    ...
    if(dvverify == dv){
      ...
      this.rutGlobal.setDv(dv);
      this.rutGlobal.setRut(rutSinGuion);
      console.log('RUT guardado:', rutSinGuion);
      
      this.router.navigate(['sectoneoption']);
    }else{
