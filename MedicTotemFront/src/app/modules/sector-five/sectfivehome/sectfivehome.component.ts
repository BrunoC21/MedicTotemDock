...
@Component({
  selector: 'app-sectfivehome',
  templateUrl: './sectfivehome.component.html',
  styleUrls: ['./sectfivehome.component.css']
})
export class SectfivehomeComponent implements OnInit{
  content?: string;
  rutCorrecto=true;  

  constructor(private userService: UserService, private rutGlobal: RutGlobalService, 
  ...
  guardarRut(): void {
    ...
    if(dvverify == dv){
      ...
      this.rutGlobal.setDv5(dv);
      this.rutGlobal.setRut5(rutSinGuion);
      console.log('RUT guardado:', rutSinGuion);
      
      this.router.navigate(['sectfiveoption']);
    }else{
